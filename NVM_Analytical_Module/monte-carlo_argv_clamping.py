...
LRS_vol = 0.57 #voltage
HRS_vol = 0.63 #voltage
...
def pdf_current(x, vol, mu, sig): ##current
...
def integral(a, b, func, vol, mu, sig):
  h = (b-a)/float(10000)
  xk = [a + i*h for i in range (1,10000)] 
  xk = np.array(xk)
  pdf = func(xk, vol, mu, sig)
...
print(integral(0.018,0.0415, pdf_current, LRS_vol, float(cell_LRS_mu), float(cell_LRS_sig)))
print(integral(0,0.02, pdf_current, HRS_vol, float(cell_HRS_mu), float(cell_HRS_sig)))
...
def cdf_current(a, b, ind):
  ...
  for i in range(len(xk)):
    if ind == 0:
      prob = integral(a,xk[i], pdf_current, LRS_vol, float(cell_LRS_mu), float(cell_LRS_sig))
      cdf_LRS_x.append(xk[i])
      cdf_LRS_y.append(prob)
    else:
      prob = integral(a,xk[i], pdf_current, HRS_vol, float(cell_HRS_mu), float(cell_HRS_sig))
...
pk.dump(m, open('3sig_clamping/Err_file_mean_'+HRS_mu+'_var_'+portion+'_offset_'+offset+'_SA_'+SA_bit+'.p', 'wb'))

with open('3sig_clamping/Err_file_mean_'+HRS_mu+'_var_'+portion+'_offset_'+offset+'_SA_'+SA_bit+'.pkl', 'wb') as f:
