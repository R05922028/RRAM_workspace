...
for RRAM_size in range(1, RRAM_cnt+1):
  ...
  for idx in range(ref_cnt):
    ...
    margin_dv_half = margin_ref-0.5*sensing_offset
    margin_dv = margin_ref-sensing_offset
    margin_dv_half_neg = margin_ref+0.5*sensing_offset
    margin_dv_neg = margin_ref+sensing_offset
    next_margin_ref=float((next_ref + right_ref)/2)
    front_margin_ref=float((front_ref + left_ref)/2)
    print(margin_ref)
    ...
    for current in range(RRAM_size+1):
      ...
      if current < idx_2:
        for i in range(len(Data[current])):
          if Data[current][i][0] > margin_dv and Data[current][i][0] < margin_dv_half:
            if np.random.rand(1) < 0.05:
              cnt_left += 1
          elif Data[current][i][0] > margin_dv_half and Data[current][i][0] < margin_ref:
            if np.random.rand(1) < 0.25:
              cnt_left += 1
          elif Data[current][i][0] > margin_ref and Data[current][i][0] < margin_dv_half_neg:
            if np.random.rand(1) < 0.75:
              cnt_left += 1
          elif Data[current][i][0] > margin_dv_half_neg and Data[current][i][0] < margin_dv_neg:
            if np.random.rand(1) < 0.95:
              cnt_left += 1
          elif Data[current][i][0] > margin_dv_neg and Data[current][i][0] < next_margin_ref:
            cnt_left += 1
        ...
        for j in range(len(Data[current])):
          if Data[current][j][0] < margin_dv_neg and Data[current][j][0] > margin_dv_half_neg:
            if np.random.rand(1) > 0.95:
              cnt_right += 1
          elif Data[current][j][0] < margin_dv_half_neg and Data[current][j][0] > margin_ref:
            if np.random.rand(1) > 0.75:
              cnt_right += 1
          elif Data[current][j][0] < margin_ref and Data[current][j][0] > margin_dv_half:
            if np.random.rand(1) > 0.25:
              cnt_right += 1
          elif Data[current][j][0] < margin_dv_half and Data[current][j][0] > margin_dv:
            if np.random.rand(1) > 0.05:
              cnt_right += 1
          elif Data[current][j][0] < margin_dv and Data[current][j][0] > front_margin_ref:
            cnt_right += 1
  ...
  if int(RRAM_size)>level_SA:
    for idx_2 in range(ref_cnt+1, int(RRAM_size)+1):
      ...
      for j in range(len(Data[idx_2])):
        if Data[idx_2][j][0] < margin_dv_neg and Data[idx_2][j][0] > margin_dv_half_neg:
          if np.random.rand(1) > 0.95:
            cnt_right += 1
        elif Data[idx_2][j][0] < margin_dv_half_neg and Data[idx_2][j][0] > margin_ref:
          if np.random.rand(1) > 0.75:
            cnt_right += 1
        elif Data[idx_2][j][0] < margin_ref and Data[idx_2][j][0] > margin_dv_half:
          if np.random.rand(1) > 0.25:
            cnt_right += 1
        elif Data[idx_2][j][0] < margin_dv_half and Data[idx_2][j][0] > margin_dv:
          if np.random.rand(1) > 0.05:
            cnt_right += 1
        elif Data[idx_2][j][0] < margin_dv and Data[idx_2][j][0] > front_margin_ref:
          cnt_right += 1
